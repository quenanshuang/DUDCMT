...
class ISICDataset(Dataset):
    def __init__(self, image_path=None, stage='train', image_size=256, is_augmentation=False):
        ...
        if self.stage == 'train':
            self.image_files = sorted([os.path.join(image_path, x, y, z)
                                for x in os.listdir(image_path) if x.endswith('training1')
            ...
            self.label_files = sorted([os.path.join(image_path, x, y, z)
                                for x in os.listdir(image_path) if x.endswith('training1')
                                for y in os.listdir(os.path.join(image_path, x)) if y.endswith('1st_manual')
                                for z in os.listdir(os.path.join(image_path, x, y))])
        else:
            self.image_files = sorted([os.path.join(image_path, x, y, z)
                                for x in os.listdir(image_path) if x.endswith('test1')
                                for y in os.listdir(os.path.join(image_path, x)) if y.endswith('images')
                                for z in os.listdir(os.path.join(image_path, x, y))])
            self.label_files = sorted([os.path.join(image_path, x, y, z)
                                for x in os.listdir(image_path) if x.endswith('test1')
        ...
        self.post_transform1 = self.post_transform1()
    def __getitem__(self, item):
        # print("self.image_files[item].split(self.sep)[-1]")
        # print(self.image_files[item].split(self.sep)[-1][10:-4])
        # print("self.label_files[item].split(self.sep)[-1]")
        # print(self.label_files[item].split(self.sep)[-1][9:-4])
        # assert self.image_files[item].split(self.sep)[-1][:-4] == self.label_files[item].split(self.sep)[-1][:-4]
        
        # assert self.image_files[item].split(self.sep)[-1][0:1] == self.label_files[item].split(self.sep)[-1][0:1]
        assert self.image_files[item].split(self.sep)[-1][10:-4] == self.label_files[item].split(self.sep)[-1][9:-4]
        ...
        if self.stage == 'train':
            ...
            image = Image.fromarray(image).convert('RGB').resize((512, 512)) 
            label = Image.fromarray(label).convert('L').resize((512, 512)) 
            image = self.post_transform1(image)
            # label = Image.fromarray(label).convert('L').resize((690, 690))
            label = torch.from_numpy(np.array(label)).unsqueeze(0)
        else:
            image, label = self.post_transform(image), self.label_transform(label)
            label = torch.from_numpy(np.array(label)).unsqueeze(0)
    ...
    def post_transform1(self):
        return Compose([
            Resize([512, 512], InterpolationMode.BILINEAR),
            ToTensor(),
            Normalize([0.485, 0.456, 0.406],
                      [0.229, 0.224, 0.225])
        ])
...
class ISICDataset1(Dataset):
    def __init__(self, image_path=None, stage='train', image_size=256, is_augmentation=False):
        super(ISICDataset1, self).__init__()
        self.image_size = image_size
        self.sep = '\\' if sys.platform[:3] == 'win' else '/'
        self.stage = stage
        self.is_augmentation = is_augmentation
